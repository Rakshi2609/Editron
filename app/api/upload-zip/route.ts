...
const MAX_TOTAL_UNCOMPRESSED_SIZE = 100 * 1024 * 1024; // 100MB
...
async function zipToTemplateFolder(zip: JSZip): Promise<TemplateFolder> {
    ...
    zip.forEach((relativePath, file) => {
        if (!file.dir) {
            ...
            if (size > MAX_SINGLE_FILE_SIZE) {
                console.warn(`Skipping large file in ZIP: ${relativePath} (${size} bytes)`);
                return;
            }
    ...
    if (totalSize > MAX_TOTAL_UNCOMPRESSED_SIZE) {
        throw new Error(`Total uncompressed size exceeds limit (${MAX_TOTAL_UNCOMPRESSED_SIZE} bytes)`);
    }
